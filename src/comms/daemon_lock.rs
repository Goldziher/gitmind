...
//!
//! Two safeguards against the process/memory leak class, layered on top of the existing per-comms-dir
//! guards (the UDS bind and the store flock):
//!
//! 1. **[`DaemonLock`]** — an exclusive `fs2` flock on `<comms_dir>/daemon.lock`, acquired *before*
//!    the socket bind so a redundant daemon converges (exits 0) without first unlinking the live
//!    daemon's socket. Holding it writes `<comms_dir>/daemon.pid` and a machine-registry entry; both
//!    are removed on `Drop`. This is the authoritative "one daemon per comms dir" gate, uniform
//!    across Unix and Windows (a real lock, not the Unix-only socket-inode watchdog).
//!
//! 2. **The machine ceiling** — every live daemon registers a pidfile under `<data_home>/daemons/`.
//!    [`count_live_daemons`] tallies them, pruning dead holders via [`pid_is_live`], and
//!    [`ensure_daemon`](super::singleton::ensure_daemon) refuses to spawn past [`max_live_daemons`].
//!    It is keyed on the resolved data home (`BASEMIND_DATA_HOME`), so it is a **production** ceiling
//!    on a shared cache — NOT the test guard. Tests isolate the data home per process, so a suite's
//!    daemons never accumulate here; their leak safety comes from the bootstrap reaper and the
//!    kill-and-wait test harness, not this count.
...
/// The flock file within a comms dir. Distinct from the store's `.lock` — this one is held for the
/// daemon-process lifetime and gates the singleton decision before bind.
const DAEMON_LOCK_FILE: &str = "daemon.lock";
/// The pidfile within a comms dir naming the current daemon (for a contender's converge log).
...
/// Default machine-wide ceiling on concurrently live daemons before a spawn is refused. Well above
/// any legitimate need (one per user is the norm; a handful covers isolated worktrees/data homes),
/// low enough to turn a runaway into a loud error long before it exhausts the process table.
...
/// One live daemon's registry entry. Written to both `<comms_dir>/daemon.pid` and the machine
/// registry file so a contender can name the holder and the ceiling can prune dead ones.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DaemonRecord {
    /// The daemon process id.
    pub pid: u32,
    /// The comms dir this daemon owns.
...
/// The registry pidfile for a comms dir within a given machine dir: `<machine_dir>/<hash>.pid`.
fn machine_registry_path_in(machine_dir: &Path, comms_dir: &Path) -> PathBuf {
    machine_dir.join(format!("{}.pid", workspace_key(comms_dir)))
}
...
/// Read a daemon record from a pidfile, or `None` when absent/corrupt.
fn read_record(path: &Path) -> Option<DaemonRecord> {
    serde_json::from_slice(&std::fs::read(path).ok()?).ok()
}

/// The live daemons registered on this machine (under the resolved data home), with the pidfiles of
/// any that have died pruned as a side effect. Best-effort: an unreadable registry dir yields empty.
/// Powers the ceiling ([`count_live_daemons`]), `comms doctor`, and `comms stop --all`.
...
/// Count the live daemons on this machine, pruning dead pidfiles. See [`live_daemons`].
pub fn count_live_daemons() -> usize {
    live_daemons().len()
}
...
pub struct DaemonLock {
    ...
    _lock: File,
    /// `<comms_dir>/daemon.pid`, removed on drop.
    pid_path: PathBuf,
    /// `<data_home>/daemons/<hash>.pid`, removed on drop.
    machine_path: PathBuf,
}

/// The result of trying to become the daemon for a comms dir.
#[derive(Debug)]
pub enum DaemonLockOutcome {
    /// We are now the sole daemon; hold the returned lock for the process lifetime.
    Acquired(DaemonLock),
    /// Another live daemon already holds this comms dir. Converge: exit 0. The record names it when
    /// the pidfile was readable.
    AlreadyHeld(Option<DaemonRecord>),
}

impl DaemonLock {
    /// Try to acquire single-ownership of `comms_dir` for a daemon at `version`.
    ///
    ...
    pub fn acquire(comms_dir: &Path, version: &str) -> std::io::Result<DaemonLockOutcome> {
        Self::acquire_at(comms_dir, version, &daemons_dir())
    }

    /// [`acquire`](Self::acquire) registering into an explicit machine dir. Injected so unit tests
    /// exercise the lock + registry against a temp dir without touching `BASEMIND_DATA_HOME`.
    fn acquire_at(comms_dir: &Path, version: &str, machine_dir: &Path) -> std::io::Result<DaemonLockOutcome> {
        let lock_path = comms_dir.join(DAEMON_LOCK_FILE);
        ...
        if file.try_lock_exclusive().is_err() {
            let pid_path = comms_dir.join(DAEMON_PID_FILE);
        ...
        let pid_path = comms_dir.join(DAEMON_PID_FILE);
        let machine_path = machine_registry_path_in(machine_dir, comms_dir);
...
mod tests {
    ...
    fn acquire_admits_one_owner_then_reports_already_held() {
        ...
        let first = DaemonLock::acquire_at(comms.path(), "9.9.9", machine.path()).expect("first acquire");
        assert!(
            matches!(first, DaemonLockOutcome::Acquired(_)),
            "the first daemon wins the lock"
        );
        match DaemonLock::acquire_at(comms.path(), "9.9.9", machine.path()).expect("second acquire") {
    ...
    fn count_prunes_dead_holders_and_counts_live_ones() {
        let machine = tempfile::tempdir().expect("machine tempdir");
        let comms = tempfile::tempdir().expect("comms tempdir");
        let held = DaemonLock::acquire_at(comms.path(), "9.9.9", machine.path()).expect("acquire");
        ...
        let dead = DaemonRecord {
            pid: 0x7FFF_FFFE,
            comms_dir: PathBuf::from("/nonexistent"),
    ...
    fn drop_releases_the_lock_and_removes_the_registry_entry() {
        ...
        match DaemonLock::acquire_at(comms.path(), "9.9.9", machine.path()).expect("re-acquire after drop") {
