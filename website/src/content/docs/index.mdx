...
<CardGrid>
    <Card title="Answer from structure" icon="magnifier">
        `code` (`mode: outline`) a file before you open it, `code` (`mode: symbols`) instead of
        grep, `code` (`mode: references`) instead of grepping call sites. Pointers, not file
        bodies.
    </Card>
    <Card title="Ask history directly" icon="git-branch">
        `git` (`mode: recent`, `mode: blame_symbol`, `mode: touching`, `mode: diff_outline`) — git
        intelligence at symbol resolution, in tens of microseconds.
    </Card>
