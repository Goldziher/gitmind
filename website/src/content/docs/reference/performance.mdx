...
Every threshold sits well below the `limit` its capture call passes, so a canary can never be
satisfied merely by hitting the cap — and never fails because upstream churn moved a count.
