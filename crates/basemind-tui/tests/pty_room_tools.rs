//! PTY end-to-end: the model-facing `room_post` tool. A scripted turn calls `room_post`, which
...
/// A scenario whose first turn calls `room_post` (gated), then a closing turn that stops. A room is
/// declared so the tool has somewhere to post.
const SCENARIO: &str = r#"{
    "user": "ping the room",
    "turns": [
        { "tools": [ { "id": "c1", "name": "room_post", "args": { "text": "PEER-PING" } } ] },
        { "text": "Pinged." }
    ],
...
fn room_post_tool_is_permission_gated_then_runs() {
    ...
    // room_post is a comms claim, so it prompts rather than auto-running. ~keep
