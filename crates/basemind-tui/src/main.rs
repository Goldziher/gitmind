...
//!
//! Usage: `basemind-tui ["<initial prompt>"] [--root <path>]`. The model is read from
//! `BASEMIND_AGENT_MODEL` (default `anthropic/claude-sonnet-4`) and the API key from
//! `ANTHROPIC_API_KEY`.
...
use basemind_agent::{AgentClient, AgentCommand, Session, ToolRegistry, in_proc_channel};
...
/// Command-line arguments: an optional initial prompt and a repo root.
struct Args {
    prompt: Option<String>,
    root: PathBuf,
}

/// Parse `[prompt] [--root <path>]` from the process arguments.
fn parse_args() -> Args {
    let mut prompt = None;
