...
|---|---|---|
| **Code intelligence** | Find where things are defined, what calls what, who implements what, how calls chain, and the overall architecture (hub modules + dependency cycles) — with **precise, scope- and import-aware resolution** (JS/TS via oxc, Python & Java via in-tree [stack-graphs](#how-it-works)) layered over [300+ languages](#how-it-works). | `outline` · `search_symbols` · `find_references` · `find_callers` · `goto_definition` · `call_graph` · `architecture_map` · `find_implementations` · `find_files` · `workspace_grep` |
